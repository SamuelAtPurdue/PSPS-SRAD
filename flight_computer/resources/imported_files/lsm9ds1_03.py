...
def _update(self):
	...
	self.data.timestamp = time.time() - t0

	self.data.accel = list(self.accelerometer)
	self.data.accel_angles = _absolute_angles(self.data.accel)
	self.data.accel_mgtd = _magnitude(self.data.accel)
	self.data.gyro = list(self.gyroscope)
	self.data.gyro_angles = _absolute_angles(self.data.gyro)
	self.data.gyro_mgtd = _magnitude(self.data.gyro)
	self.data.mag = list(self.magnetometer)
	self.data.mag_angles = _absolute_angles(self.data.mag)
	self.data.mag_mgtd = _magnitude(self.data.mag)
